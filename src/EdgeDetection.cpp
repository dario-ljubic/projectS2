...
std::pair<double,
          double> EdgeDetection::FindThresholdValue(double include_factor,
                                                    double img_median,
                                                    int num_bits) {

  double upper_threshold =
      std::fmin(std::pow(2, num_bits), (1 + include_factor) * img_median);
...
double EdgeDetection::CalculateImageMedian(cv::Mat &Input, int num_bits) {
  ...
  cv::Mat hist;
  // Calculate histogram of only 1-st image, and just the 0-th channel, do not
  // use the mask output histogram, histogram dimensionality,
  calcHist(&Input,
           1,
           0,
           cv::Mat(),
...
int EdgeDetection::CannyEdgeDetection(cv::Mat &input_image,
                                      cv::Mat &output_edge_map,
                                      int aperature_size,
                                      bool L2gradient) {
  ...
  std::pair<double, double>
      canny_thresholds = FindThresholdValue(0.2, img_median, num_bits);
  cv::Canny(blurred_image,
            output_edge_map,
            canny_thresholds.first,
            canny_thresholds.second,
            aperature_size,
            L2gradient);

  // check if image has depth or whatever and return 0 if everything is okay
}
